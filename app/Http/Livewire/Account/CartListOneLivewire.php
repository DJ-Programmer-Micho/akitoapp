...
class CartListOneLivewire extends Component
{
    ...
    public function loadCartList()
    {
        ...
        $this->cartListItems = CartItem::where('customer_id', Auth::guard('customer')->id())
            ->with(['product' => function ($query) use ($locale) {
                ...
                }, 'variation','categories','categories.categoryTranslation','variation.images' => function ($imageQuery) {
    ...
    //     // Initialize total price and total discount
    //     $this->totalListPrice = 0;  // Total original price (no discount)
    //     $this->totalDiscount = 0;   // Total discount amount

    //     // Loop through the cart items to calculate total original price and discount
    //     foreach ($this->cartListItems as $item) {
    //         // Retrieve the original price and current price from the product's variation
    //         $productPrice = $item['product']['variation']['price'];
        
    //         // Check if the product is on sale and adjust the discount calculation
    //         if (!empty($item['product']['variation']['on_sale'])) {
    //             $discountPrice = $item['product']['variation']['discount'] ?? $productPrice;
    //             $this->totalDiscount += ($productPrice - $discountPrice) * $item['quantity']; // Add discount to total discount
    //         }

    //         // Add the original price multiplied by quantity to the totalListPrice
    //         $this->totalListPrice += $item['quantity'] * $productPrice;
    //     }
    // }
    ...
    {
        return view('mains.components.livewire.account.cart-list-one', [
