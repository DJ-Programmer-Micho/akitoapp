                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-md-3">
                                    <select wire:model="images.{{ $index }}.category_id" class="form-control" wire:change="fetchSubCategories({{ $index }})">
                                ...
                                <div class="col-md-3">
                                    <select class="form-control" wire:model="images.{{ $index }}.sub_category_id">
                                        <option value="">{{ __('Select Subcategory') }}</option>
                                        @foreach($subCategories[$index] ?? [] as $subCategory)
                                            <option value="{{ $subCategory['id'] }}" @if($subCategory['id'] == ($image['sub_category_id'] ?? null)) selected @endif>
                                                {{ $subCategory['sub_category_translation']['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('images.' . $index . '.sub_category_id') <span class="text-danger">{{ $message }}</span> @enderror
                                </div>
                        ...
                        <button type="submit" class="btn btn-primary mt-3" wire:click="saveSettings">{{__('Save Settings')}}</button>
                    </div>
                </div>
            </div>
        </div>
