                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <div class="row">
                            @forelse ($base64Images[$lang] ?? [] as $index => $imageData)
                                <div wire:key="base64-{{ $lang }}-{{ $index }}" class="col-4 mb-3">
                                    <div wire:ignore.self class="img-container position-relative">
                                        ...
                                        <input type="number" wire:model.defer="base64Images.{{ $lang }}.{{ $index }}.priority" class="form-control mt-2" value="{{ $imageData['priority'] ?? 0 }}" />
                                    </div>
                                </div>
                            @empty
                            ...
                            @foreach ($uploadedImages[$lang] ?? [] as $index => $imageData)
                                <div wire:key="uploaded-{{ $lang }}-{{ $index }}" class="col-4 mb-3">
                                    <div wire:ignore.self class="img-container position-relative">
                                        <img src="{{ app('cloudfront') . $imageData['filename'] }}" alt="{{ $lang }}" class="img-thumbnail w-100">
                                        ...
                                        <input type="number" wire:model.defer="uploadedImages.{{ $lang }}.{{ $index }}.priority" class="form-control mt-2" value="{{ $imageData['priority'] ?? 0 }}" />
                                    </div>
                                </div>
                            @endforeach
                        </div>
