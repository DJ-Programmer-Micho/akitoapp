<div class="page-content">
    @include('super-admins.pages.comingsoon.soon-form',[$title = "Brand Image"])
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0">{{__('Coming Soon Brands')}}</h4>

                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="javascript: void(0);">{{__('Dashboard')}}</a></li>
                            <li class="breadcrumb-item active">{{__('Soons')}}</li>
                        </ol>
                    </div>
        ...
        <div class="row">
                                ...
                                <table class="table table-bordered table-striped table-hover table-sm">
                                    ...
                                    <tbody>
                                        @forelse($tableData as $data)
                                            <tr wire:key="soon-{{ $data->id }}">
                                                <td class="@empty($data->coming_soon_translation->name) text-danger @endif align-middle">
                                                    <div class="d-flex align-items-center">
                                                        <div class="flex-shrink-0 me-2">
                                                            <img src="{{ app('cloudfront').$data->image }}" alt="{{ $data->coming_soon_translation->name }}" class="img-fluid" style="max-width: 60px; max-height: 60px; object-fit: cover; background-color: #f8d7da;">
                                                        </div>
                                                        <div>
                                                            <h6 class="mb-0">{{ $data->coming_soon_translation->name ?? 'unKnown' }}</h6>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <span>
                                                        <div class="dropdown"><button
                                                            ...
                                                            <ul class="dropdown-menu dropdown-menu-end" style="">
                                                                ...
                                                                <li><button type="button" class="dropdown-item edit-list" data-bs-toggle="modal" data-bs-target="#updateSoonModal" wire:click="editSoon({{ $data->id }})">
                                                                    <i class="fa-regular fa-pen-to-square me-2"></i>{{__('Edit')}}</button>
                                                                </li>
                                                                <li class="dropdown-divider"></li>
                                                                <li>
                                                                    <button type="button" class="dropdown-item edit-list" data-bs-toggle="modal" data-bs-target="#deleteSoonModal" wire:click="removeSoon({{ $data->id }})">
            ...
            <div @if($de == 0) wire:ignore @endif class="col-xl-3 col-lg-4">
                <div class="card">
                    <div class="card-header">
                        <form wire:submit.prevent="saveSoon">
                            <div class="d-flex mb-3">
                                <div class="flex-grow-1">
                                    <h5 class="fs-16">{{ __('Add New Brand') }}</h5>
                                </div>
                                <div class="flex-shrink-0">
                                    <button type="submit" class="btn btn-success">
                                        <i class="ri-add-line align-bottom me-1"></i> {{__('Add Brand')}}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="filter-choices-input">
                                @foreach ($filteredLocales as $locale)
                                    <div class="mb-3">
                                        <label for="soons.{{ $locale }}" class=" @if($locale != 'en') ar-shift @endif">{{__('In ' . $locale . ' Language')}}</label>
                                        <input type="text" 
                                            class="form-control @if($locale != 'en') ar-shift @endif
                                            @error('soons.' . $locale) is-invalid @enderror
                                            @if(!$errors->has('soons.' . $locale) && !empty($soons[$locale])) is-valid @endif"
                                            wire:model="soons.{{ $locale }}" placeholder="{{__('Brand Name')}}">
                                        @error('soons.' . $locale)
